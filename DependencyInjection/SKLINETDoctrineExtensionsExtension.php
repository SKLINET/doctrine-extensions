...
namespace SKLINET\DoctrineExtensionsBundle\DependencyInjection;
...
class SKLINETDoctrineExtensionsExtension extends Extension
{
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        $container->setParameter('sklinet_doctrine_extensions.default_locale', $config['default_locale']);
        $container->setParameter('sklinet_doctrine_extensions.translation_fallback', $config['translation_fallback']);
        $container->setParameter('sklinet_doctrine_extensions.persist_default_translation', $config['persist_default_translation']);
        $container->setParameter('sklinet_doctrine_extensions.skip_translation_on_load', $config['skip_translation_on_load']);
        ...
        if (isset($loaded['uploadable'])) {
            ...
            $container->setParameter('sklinet_doctrine_extensions.default_file_path', $uploadableConfig['default_file_path']);
            $container->setParameter('sklinet_doctrine_extensions.uploadable.default_file_info.class', $uploadableConfig['default_file_info_class']);
            $container->setParameter(
                'sklinet_doctrine_extensions.uploadable.validate_writable_directory',
                $uploadableConfig['validate_writable_directory']
            );

            if ($uploadableConfig['default_file_path']) {
                $container->getDefinition('sklinet_doctrine_extensions.listener.uploadable')
            ...
            if ($uploadableConfig['mime_type_guesser_class']) {
                ...
                $container->setParameter(
                    'sklinet_doctrine_extensions.uploadable.mime_type_guesser.class',
        ...
        foreach ($config['class'] as $listener => $class) {
            $container->setParameter(sprintf('sklinet_doctrine_extensions.listener.%s.class', $listener), $class);
        }
    }
    ...
    private function processObjectManagerConfigurations(array $configs, ContainerBuilder $container, LoaderInterface $loader, array &$loaded, $doctrineSubscriberTag)
    {
        ...
        foreach ($configs as $name => $listeners) {
            foreach ($listeners as $ext => $enabled) {
                ...
                $definition = $container->getDefinition(sprintf('sklinet_doctrine_extensions.listener.%s', $ext));
